...
def _dispatch_body(task: dict) -> str:
    ...
    if phase == "proofread.start":
        return "进入 proofread blocker 校稿阶段。只有 blocker 清零后，才允许 publish。"
    if phase == "proofread.issue.submit":
        return "请只提交自己负责板块的 proofread issue。"
    if phase == "proofread.decision":
        return "请对 proofread issue 做 accept/reject 决策，并生成 revision patch。"
    if phase == "proofread.recheck":
        return "请复查修订稿，确认 blocker 是否真正关闭。"
...
def _task_body(task: dict) -> str:
    ...
    if task["phase"] == "draft.review.summarize":
        ...
        if html_path:
            return f"{summary}\n校稿总结入口：{html_path}"
        return summary
    if task["phase"] == "proofread.issue.submit":
        return result.get("message_body", "已提交 proofread issue。")
    if task["phase"] == "proofread.decision":
        ...
        if html_path:
            return f"{summary}\n决策入口：{html_path}"
        return summary
    if task["phase"] == "proofread.recheck":
        return result.get("message_body", "已完成 proofread recheck。")
...
def build_conversation_entries(run_id: str) -> list[dict]:
    ...
    for row in task_rows:
        ...
        if task["phase"] in {"draft.review.start", "retrospective.start", "proofread.start"}:
            entries.append(
                {
        ...
        if task["phase"] in {"draft.compose", "material.submit", "draft.review.summarize", "proofread.issue.submit", "proofread.decision", "draft.revise", "proofread.recheck", "report.publish", "product.test", "product.benchmark", "product.report", "retrospective.summary", "agent.self_optimize"}:
